        ...
        TimelineView(.animation(paused: !animated)) { context in
            ...
            Canvas { context, size in
                ...
                context.drawLayer { context in
                    ...
                    context.scaleBy(x: scale, y: scale)
                    context.draw(Image("header/layer/8 Road"), at: .zero)
                }
                
                // MARK: Truck
                context.drawLayer { context in
                    context.translateBy(x: centerX, y: 320 * scale)
                    context.scaleBy(x: scale * 0.15, y: scale * 0.15) // Scale down by 20%
...
extension BrandHeader {
    ...
    struct AnimationDirector {
        ...
        init(timeInterval: TimeInterval) {
            ...
            truckImage = Image("header/truck/Frame \(truckFrame + 1)")
        }
    }
}
